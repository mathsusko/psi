...
interface Props {
  onSelectCliente: (clienteId: string) => void // Alterado para receber clienteId
}

export function DadosCliente({ onSelectCliente }: Props) {
  // Alterado para "DadosCliente" sem a parte de filial
  const [clientes, setClientes] = useState<Cliente[]>([])
  const [clienteSelecionado, setClienteSelecionado] = useState<Cliente | null>(null)
  ...
  const handleClienteChange = (cliente: Cliente) => {
    setClienteSelecionado(cliente)
    onSelectCliente(cliente._id) // Passa apenas o clienteId
  }
    ...
    return (
      <div className="flex flex-col gap-12 w-full px-4">
        <div className="flex flex-wrap gap-6 text-xs">
          <div>
            <span className="text-zinc-500">Nome</span>
            <p>{clienteSelecionado.nomeEmpresa}</p>
          </div>
          <div>
            <span className="text-zinc-500">CNPJ/CPF</span>
            <p>{clienteSelecionado.cnpjCpf}</p>
          </div>
          <div>
            <span className="text-zinc-500">Inscrição Estadual</span>
          ...
          <div>
            <span className="text-zinc-500">Categoria</span>
            <p>{clienteSelecionado.categoria}</p>
          </div>
          <div>
            <span className="text-zinc-500">E-mail</span>
        ...
        <div className="flex flex-wrap gap-6 text-xs">
          ...
          <div>
            <span className="text-zinc-500">Número</span>
            <p>{clienteSelecionado.numeroEndereco}</p>
          </div>
          <div>
            <span className="text-zinc-500">Cidade</span>
  ...
  return (
    <div className="flex flex-col gap-4 p-4 rounded-xl bg-sidebar text-sidebar-foreground">
            ...
            <DropdownMenuContent className="p-2 border rounded-md bg-sidebar">
              {clientes.map((cliente) => (
                <DropdownMenuItem
                  key={cliente._id}
                  className="p-2 rounded-md"
                  onClick={() => handleClienteChange(cliente)} // Chama handleClienteChange ao invés de filiais
                >
                  {cliente.nomeEmpresa}
                </DropdownMenuItem>
              ))}
      ...
      {clienteSelecionado && renderClienteInfo()} {/* Exibe as informações do cliente */}
    </div>
  )
}
