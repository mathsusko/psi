...
export function useCategoria() {
  ...
  const [materialData, setMaterialData] = useState<MaterialData>({
    ...
    precoUn: '',
    precoTotal: '',
    imagem: '' // Valor inicial para imagem
  })
  const [filteredCards, setFilteredCards] = useState<any[]>([])
  ...
  const fetchMedidas = async (cardId: string) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3333/api/cards/${cardId}/itens`)
      const medidas = response.data.map((item: any) => item.medida)
      setMedidas(medidas)
    } catch (error) {
      console.error('Erro ao carregar medidas:', error)
    } finally {
  ...
  const handleCategoriaChange = (categoria: string) => {
    ...
    // Atualizar as medidas ao selecionar a categoria
    if (categoria === 'Material por Kg' || categoria === 'Material por Metros') {
      const selectedCard = cards.find((card) => card.categoria === categoria)
      if (selectedCard) {
        fetchMedidas(selectedCard._id)
      }
    }
  ...
  return {
    ...
    handleCategoriaChange,
    handleMaterialChange,
    fetchMedidas // Adicionando a função fetchMedidas no retorno
  }
}
